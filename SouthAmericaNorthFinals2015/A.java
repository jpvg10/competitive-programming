...
//Available at: https://uva.onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&category=866&page=show_problem&problem=4892
public class A {
		...
		while((u = scan.readLine()) != null){
			...
			if(isGood(u)){
				...
				for(int i = 2; i <= len; i++){
					...
					max = f > max && less(u, formed) && isGood(formed) ? f:max;
				}
	...
	static boolean isGood(String u){
		int c[] = freq(u);
		for(int k = 0; k < 10; k++){
